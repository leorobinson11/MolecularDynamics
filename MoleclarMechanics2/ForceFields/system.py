...
class System(Geometry):
    ...
    def all_strechbend(self)->list:
        #all the angles plus a reversed copy so that the there is an interaction for each bond in the angle
        self.strechbend_graph = []
        for angle_set in self.angle_graph:
            self.strechbend_graph.append(angle_set)
            self.strechbend_graph.append(angle_set[::-1])
    ...
    def all_outofplaines(self)->list:
        self.outofplaines_graph = []
        for (key, items) in self.bonded_graph.items():
            if len(items) >= 3:
                ...
